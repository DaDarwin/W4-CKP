...
export class Todo{
    ...
    get template(){
        ...
        <div id="${this.id}" class="todo m-2 p-3 border border-1 rounded  ${this.completed? 'border-done' : 'border-light'} ${this.completed? 'text-done' : 'text-light'} d-flex justify-content-between align-items-center">
            <input oninput="app.AccountController.markDone('${this.id}')" type="checkbox" ${this.completed? 'checked' : '' } class="form-check-input m-0 bg-transparent ${this.completed? 'border-done' : 'border-light'} ${this.completed? 'text-done' : 'text-light'}" title="Mark-Done">
